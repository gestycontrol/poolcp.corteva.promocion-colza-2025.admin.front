            ...
            <div class="w-100 mt-2">
                <div class="row g-2">
                    <template v-for="(valueItem, index) in ((value || []).length + (disabled ? 0 : 1))" :key="index">
                        <div class="col-12">
                            <Field :modelValue="(value || [])[index]" :preview="preview" :disabled="disabled"
                                :rest="rest" :filter="filter" :sort="sort" :type="type"
                                @update:modelValue="updateInnerModelValue(index, $event)" :maxlength="maxlength"
                                :label="computedLabelTemplate.replace('{number}', index + 1)">
...
export default {
    ...
    computed: {
        computedLabelTemplate() {
            if (this.labelTemplate) return this.labelTemplate;
            return this.i18n('Element #{number}');
        },
        expectsObject() {
